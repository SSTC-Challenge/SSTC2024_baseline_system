...
from dataset.dataset import WavDataset
from nemo.core.optim.lr_scheduler import CosineAnnealing
from omegaconf import OmegaConf

parser = argparse.ArgumentParser(description='sstc2024 baseline system')
...
dataset = WavDataset(utt2wav, utt2spk, args.fs, is_aug=args.data_aug, snr=args.snr_range, noise_list=noise_list,channel=1)
...
# validation dataset
val_dataset = WavDataset([line.split() for line in open('data/%s/%s.scp' % (args.val_data_name, args.data_name[0]))], fs=args.fs)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (feats, key) in enumerate(train_loader):
            ...
            with torch.cuda.amp.autocast(enabled=False):
                outputs = classifier(model(featCal(feats), feats_len), key)
        ...
        save_checkpoint('exp/%s' % save_dir, epoch, model, classifier, optimizer, scheduler, scaler)
...
def validate(model, val_dataloader, epoch, save_dir):
    ...
    with torch.no_grad():
        for j, (feat, utt) in enumerate(val_dataloader):
            ...
            embd_dict[utt[0]] = model(featCal(feat), feat_len).cpu().numpy()
